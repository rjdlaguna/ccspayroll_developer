...
namespace CCSPayrollBillingSystem.Scripts
{
    public class QueryProcessor
    {
                                               ...
                                               int jobID, int empStatus, int deductionID, Action onSuccess, Action onFailure)
        {
            ...
            string hdate = datehired.ToString("yyyy-MM-dd");
            string ecdate = endofcontract.ToString("yyyy-MM-dd");
            string sqlInsert = "INSERT INTO tblEmployee(EmpFirstName,EmpMiddleName,EmpLastName,EmpHomeAddress,EmpContactNo,EmpBirthDate,EmploymentDate,EndOfContract,JobID,EmpStatus,DeductionID) "
                                + " VALUES('" + firstname + "','" + middlename + "','" + lastname + "','"
                                                                + homeaddress + "', '" + contactno + "','" + bdate + "','"
                                                                + hdate + "','" + ecdate + "','" + jobID + "','" +
        ...
        //Add Loan Payment
        public void ExecuteSQLLoanPaymentAdd(int loanId, int payrollId, decimal amountToPay, DateTime startDate, DateTime endDate, decimal totalamountPaid, decimal remBalance, Action onSuccess, Action onFailure)
        {
            var loanPaymentDate = DateTime.Now.ToString("yyyy-MM-dd");
            string sDate = startDate.ToString("yyyy-MM-dd");
            string eDate = endDate.ToString("yyyy-MM-dd");
            string sqlInsertLoanPaymentDetails = "INSERT INTO tblLoanPayment(LoanID,AmountToPay,DateOfPayment,PayrollStartDate,PayrollEndDate,TotalLoanPayment,RemainingLoanBalance,PayrollID) " +
                                "VALUES('" + loanId + "','" + amountToPay + "','" + loanPaymentDate + "','" + sDate + "','" + eDate + "','" + totalamountPaid + "', '" + remBalance + "','" + payrollId + "')";
        ...
        public void ExecuteSQLLoanPaymentDataViewQuery(int loanId, Action onSuccess, Action onFailure)
        {
            using (connection = new DatabaseConnection(connectionString))
            {
                ...
                {
                    sqlLoanDataSearch = "SELECT AmountToPay as 'Amount Paid', DateOfPayment as 'Date of Payment', PayrollStartDate as 'Payroll Start Date', PayrollEndDate as 'Payrol Cutoff Date', TotalLoanPayment as 'Total Amount Paid', RemainingLoanBalance as 'Remaining Balance' " +
                                        "FROM tblLoanPayment";
                }
                else
                {
